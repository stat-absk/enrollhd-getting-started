...
format:
  revealjs:
    theme: [default, signature-reveal.scss, custom.scss]
    slide-number: true
    code-line-numbers: false
    code-overflow: wrap
    footer: "[stat-absk.github.io](https://stat-absk.github.io/) · [more slides and projects](https://stat-absk.github.io/stuff.html) · Enroll-HD for statisticians"
    width: 1280
    height: 720
    fig-width: 8
    fig-height: 4
...
# Figures sit on the paper, not in a white box: transparent devices and panels.
...
paper <- "#F2F3EA"
tints <- c("#E9D592", "#F1E8C5", "#DFE0D5", "#CBD3C4", "#E0E1D9")   # chalk and its neighbours
pds <- make_toy_pds()
clr <- make_toy_hdclarity()
```
...
tribble(
  ...
  gt() |>
  tab_options(table.font.size = px(18), table.background.color = paper) |>
...
ggplot() +
  ...
  scale_fill_manual(values = unname(tints), name = NULL) +
...
## Deriving motor onset within the study {.smaller}
...
#| fig-height: 3.1
steps <- tribble(
  ~x, ~when, ~label, ~detail, ~study,
  1, "up to 90 days before", "Enroll-HD\nBaseline / Follow Up", "all Enroll-HD forms\n(60 days under protocol v3)", "Enroll-HD",
  2, "day 0",                "Screening",                        "consent, enrolment, safety bloods;\nclinical scales only when no\nEnroll-HD visit is in the window", "HDClarity",
  3, "0 to 30 days later",   "Sampling",                         "fasting, lumbar puncture\n(up to 20 mL CSF), up to 50 mL\nblood, motor exam repeated", "HDClarity",
  4, "4 to 8 weeks later",   "RPT Sampling",                     "optional, about 14 % of packages\n(cycle 1 only under protocol v4)", "HDClarity"
)
ggplot(steps, aes(x, 1)) +
  annotate("segment", x = 0.75, xend = 4.25, y = 1, yend = 1, colour = "grey70") +
  geom_label(aes(label = label, fill = study), size = 4.2, label.padding = unit(0.45, "lines"), show.legend = FALSE) +
  geom_text(aes(y = 1.3, label = when), size = 3.6, colour = "grey30", fontface = "bold") +
  geom_text(aes(y = 0.62, label = detail), size = 3.1, colour = "grey30", lineheight = 0.9) +
  scale_fill_manual(values = c("Enroll-HD" = "#DFE0D5", "HDClarity" = "#E9D592")) +
  coord_cartesian(ylim = c(0.4, 1.4), xlim = c(0.5, 4.5)) +
  theme_void()
```
